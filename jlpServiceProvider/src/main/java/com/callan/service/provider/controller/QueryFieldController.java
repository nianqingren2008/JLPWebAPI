...
public class QueryFieldController {
	...
	@RequestMapping(value = "/api/QueryField/**", method = { RequestMethod.GET })
...
//		JUser user = (JUser) request.getSession().getAttribute("user");
		...
		String sql = "SELECT Filter1.SORTNO1         AS SORTNO," + "       Filter1.ID1             AS ID,"
				+ "       Filter1.CODE1           AS CODE," + "       Filter1.NAME1           AS NAME,"
				+ "       Filter1.DESCRIPTION1    AS DESCRIPTION," + "       Filter1.ACTIVEFLAG      AS ACTIVEFLAG,"
				+ "       Filter1.ID2             AS ID1," + "       Filter1.TABLECODE       AS TABLECODE,"
				+ "       Filter1.CODE2           AS CODE1," + "       Filter1.NAME2           AS NAME1,"
				+ "       Filter1.TYPE            AS TYPE," + "       Filter1.DESCRIPTION2    AS DESCRIPTION1,"
				+ "       Filter1.SORTNO2         AS SORTNO1," + "       Filter1.CONTROLTYPECODE AS CONTROLTYPECODE,"
				+ "       Filter1.NULLABLE        AS NULLABLE," + "       Filter1.DICTNAME        AS DICTNAME,"
				+ "       Filter1.QUERYFLAG       AS QUERYFLAG," + "       Filter1.SHOWFLAG        AS SHOWFLAG,"
				+ "       Filter1.SORTFLAG        AS SORTFLAG," + "       Extent3.CODE            AS CODE2,"
				+ "       Extent3.NAME            AS NAME2," + "       Extent3.ACTIVEFLAG      AS ACTIVEFLAG1"
				+ "  FROM (SELECT Extent1.ID              AS ID1," + "               Extent1.CODE            AS CODE1,"
				...
				+ "               Extent2.NAME            AS NAME2," + "               Extent2.TYPE            AS TYPE,"
				...
				+ "               Extent2.SORTFLAG        AS SORTFLAG" + "          FROM JLP.J_TABLEDICT Extent1"
				+ "         INNER JOIN JLP.J_TABLEFIELDDICT Extent2" + "            ON Extent1.CODE = Extent2.TABLECODE"
				+ "         WHERE (('" + JLPConts.ActiveFlag + "' = Extent1.ACTIVEFLAG)" + " AND ('"
				+ JLPConts.QueryFieldFlag + "' = Extent2.QUERYFLAG))) Filter1"
				+ " INNER JOIN JLP.J_CONTROLTYPEDICT Extent3" + "    ON Filter1.CONTROLTYPECODE ="
		...
		String typeDictSql = "SELECT Project2.C1           AS C1," + "       Project2.NAME         AS NAME,"
				+ "       Project2.C2           AS C2," + "       Project2.CODE         AS CODE,"
				+ "       Project2.NAME1        AS NAME1," + "       Project2.VALUE        AS VALUE,"
				+ "       Project2.TYPE         AS TYPE," + "       Project2.JLACTIVEFLAG AS JLACTIVEFLAG"
				+ "  FROM (SELECT Distinct1.NAME AS NAME," + "               1 AS C1,"
				+ "               Filter3.CODE1 AS CODE," + "               Filter3.NAME1 AS NAME1,"
				+ "               Filter3.VALUE AS VALUE," + "               Filter3.TYPE AS TYPE,"
				+ "               Filter3.JLACTIVEFLAG1 AS JLACTIVEFLAG," + "               CASE"
				+ "                 WHEN (Filter3.CODE2 IS NULL) THEN" + "                  NULL"
				+ "                 ELSE" + "                  1" + "               END AS C2"
				...
				+ "            ON ('1' = Filter3.JLACTIVEFLAG1)" + "           AND ('2' = Filter3.TYPE)"
		...
		for (Map<String, Object> obj : fields) {
			...
			fieldRelations.put(ObjectUtil.objToString(obj.get("NAME")) + "." + ObjectUtil.objToString(obj.get("NAME1")),
					dictList);
		...
		String ids = url.substring(url.lastIndexOf("/") + 1, url.length());
		if (ids.length() > 0) {
			try {
				Id = ObjectUtil.objToLong(ids);
			} catch (Exception e) {
				Id = null;
			}
		}
		if (Id != null && Id.longValue() > 0) {
			...
			String tagProjectDictsSql = "SELECT Project2.C1          AS C1," + "       Project2.ID          AS ID,"
					+ "       Project2.NAME        AS NAME," + "       Project2.DESCRIPTION AS DESCRIPTION,"
					+ "       Project2.TAGTYPE     AS TAGTYPE," + "       Project2.VALUETYPE   AS VALUETYPE,"
					+ "       Project2.USERID      AS USERID," + "       Project2.PROJECTID   AS PROJECTID,"
					+ "       Project2.CREATEDATE  AS CREATEDATE," + "       Project2.ACTIVEFLAG  AS ACTIVEFLAG,"
					+ "       Project2.TAGATTACHED AS TAGATTACHED," + "       Project2.SHOWFLAG    AS SHOWFLAG,"
					+ "       Project2.TOTALNUM    AS TOTALNUM," + "       Project2.CURRENTNUM  AS CURRENTNUM,"
					+ "       Project2.C2          AS C2," + "       Project2.ID1         AS ID1,"
					+ "       Project2.TAGID       AS TAGID," + "       Project2.VALUETYPE1  AS VALUETYPE1,"
					+ "       Project2.VALUE       AS VALUE," + "       Project2.MINVALUE    AS MINVALUE,"
					+ "       Project2.MAXVALUE    AS MAXVALUE," + "       Project2.CREATEDATE1 AS CREATEDATE1,"
					+ "       Project2.ACTIVEFLAG1 AS ACTIVEFLAG1" + "  FROM (SELECT Distinct1.ID AS ID,"
					+ "               Distinct1.NAME AS NAME," + "               Distinct1.DESCRIPTION AS DESCRIPTION,"
					+ "               Distinct1.TAGTYPE AS TAGTYPE,"
					+ "               Distinct1.VALUETYPE AS VALUETYPE," + "               Distinct1.USERID AS USERID,"
					...
					+ "               Distinct1.CURRENTNUM AS CURRENTNUM," + "               1 AS C1,"
					+ "               Join2.ID1 AS ID1," + "               Join2.TAGID AS TAGID,"
					+ "               Join2.VALUETYPE1 AS VALUETYPE1," + "               Join2.VALUE AS VALUE,"
					+ "               Join2.MINVALUE AS MINVALUE," + "               Join2.MAXVALUE AS MAXVALUE,"
					+ "               Join2.CREATEDATE1 AS CREATEDATE1,"
					+ "               Join2.ACTIVEFLAG1 AS ACTIVEFLAG1," + "               CASE"
					+ "                 WHEN (Join2.ID2 IS NULL) THEN" + "                  NULL"
					+ "                 ELSE" + "                  1" + "               END AS C2"
					...
					+ "            ON ('1' = Join2.ACTIVEFLAG2)" + "           AND ('1' = Join2.ACTIVEFLAG1)"
					+ "           AND (Join2.PROJECTID = '" + Id + "')" + "           AND (Join2.USERID = '" + userId
					+ "' ) " + "           AND (Distinct1.ID = Join2.ID2)) Project2"
					+ " ORDER BY Project2.ID          ASC," + "          Project2.NAME        ASC,"
					+ "          Project2.DESCRIPTION ASC," + "          Project2.TAGTYPE     ASC,"
					+ "          Project2.VALUETYPE   ASC," + "          Project2.USERID      ASC,"
					+ "          Project2.PROJECTID   ASC," + "          Project2.CREATEDATE  ASC,"
					+ "          Project2.ACTIVEFLAG  ASC," + "          Project2.TAGATTACHED ASC,"
					+ "          Project2.SHOWFLAG    ASC," + "          Project2.TOTALNUM    ASC,"
					+ "          Project2.CURRENTNUM  ASC," + "          Project2.C2          ASC";
			...
			for (Map<String, Object> map : tagProjectDicts) {
				...
				for (Map<String, Object> map1 : typeDict) {
					...
					if (ObjectUtil.objToString(map.get("TYPE")).equals(name)) {
						String[] strArr = new String[] { ObjectUtil.objToString(map1.get("NAME")),
								ObjectUtil.objToString(map1.get("VALUE")) };
						dictList.add(strArr);
					}
				}
				fieldRelations.put("PROJECTTAGS.TAG_" + map.get("ID"), dictList);
				fieldType.put("PROJECTTAGS.TAG_" + map.get("ID"),
						"1".equals(ObjectUtil.objToString(map.get("VALUETYPE"))) ? "string" : "number");
